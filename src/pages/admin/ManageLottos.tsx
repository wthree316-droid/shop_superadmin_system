                  ...
                  <tr key={lotto.id} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="p-4 text-center">
                      <div className="w-10 h-10 mx-auto rounded-lg bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center">
                        ...
                        decoding="async"
                        alt={lotto.name} 
                        className="w-full h-full object-cover" /> : <span className="text-[10px] font-bold text-slate-300">NO</span>}
                      </div>
                    </td>
                    <td className="p-4">
              ...
              <div key={lotto.id} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 relative overflow-hidden">
                  <div className="flex items-start gap-4 mb-3">
                      <div className="w-14 h-14 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden shrink-0">
                          ...
                          decoding="async"
                          alt={lotto.name}  
                          className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xs font-bold text-slate-300">NO IMG</div>}
                      </div>
              ...
              <form id="lotto-form" onSubmit={handleSaveLotto} className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="w-full md:w-1/3 flex flex-col gap-2">
                        ...
                        >
                            {isUploading ? <Loader2 className="animate-spin text-blue-500" /> : formData.img_url ? 
                                <img src={formData.img_url} className="w-full h-full object-cover" /> : 
                                <div className="text-center p-4"><UploadCloud className="mx-auto mb-1 text-slate-400" /><span className="text-[10px] text-slate-400">แตะเพื่ออัปโหลด</span></div>
                            }
                        </div>
